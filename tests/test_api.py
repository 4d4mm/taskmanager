...
def test_post_missing_title(client: TestClient):
    response = client.post(
        "/tasks",
        json={"description": "Test description", "completed": True}
    )
...
def test_post_invalid_completed_type(client: TestClient):
    response = client.post(
        "/tasks",
        json={"title": "Test Title", "description": "Test description", "completed": 123123}
    )
    assert response.status_code == 422
...
def test_put_invalid_completed_type(client: TestClient):
    ...
    task_id = create_response.json()["id"]
    response = client.put(f"/tasks/{task_id}", json={"title": "Test Title", "description": "Test description", "completed": 123123})
    assert response.status_code == 422
    assert "completed" in response.json()["detail"][0]["loc"]
...
def test_patch_invalid_completed_type(client: TestClient):
    ...
    task_id = create_response.json()["id"]
    response = client.patch(f"/tasks/{task_id}", json={"title": "Test Title", "description": "Test description", "completed": 123123})
    assert response.status_code == 422
    assert "completed" in response.json()["detail"][0]["loc"]
